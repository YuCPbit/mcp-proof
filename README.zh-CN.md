...
**一条命令审计任意 MCP 服务器——交给客户一份签名的、可复现的交付报告，外加一套留在客户仓库里持续把关的 CI 回归套件。**
...
- 🔍 **15 项线级协议检查** —— mcp-proof 直接对服务器说原始 JSON-RPC，验证的是真正走线的内容：握手完整性、精确错误码、工具 schema 合法性、结构化输出正确性、stdout 卫生、分页安全，以及规范版本是否为最新。
...
- 🔁 **可复现是设计出来的** —— 零 LLM 调用、零 API key、零抖动。相同的服务器行为产生相同的报告指纹：验收靠验证，不靠信任。
- 🧯 **对生产服务器安全** —— 建立基线时默认跳过写型工具（`write_*`、`delete_*`、`exec` 等），需要时用 `--include-destructive` 显式放行；`--edge-cases` 额外把超长、空串、注入形态的输入也纳入基线。
...
- 可选的语义车道（LLM 评分，兼容 promptfoo 断言格式），供同时需要语义级检查的团队使用——核心始终保持确定性。
