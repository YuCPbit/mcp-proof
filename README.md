...
mcp-proof verify report.json                                   # prove the report wasn't edited after the audit
```
...
- **107 tests**, including an adversarial suite that attacks the auditor itself: violations hidden on page 2 of paginated listings, tampered fixtures and manifests, drift classes that used to slip through, invalid synthesized baselines.
...
- **Fail-closed by design**: a broken pagination walk, a tampered fixture, a missing baseline or an internal auditor error each stop the audit loudly (exit `2`) — never a silently smaller audit, and never evidence against the target.
- **Offline-verifiable reports**: `mcp-proof verify report.json` recomputes both fingerprints from the report's own fields and fails if anything was edited after the audit ran.
...
| | |
|---|---|
| **Current — v0.7.1** | Release hardening: auditor-error taxonomy (`INCONCLUSIVE` ≠ target failure, exit `2`), fail-closed baselines (`--record-if-missing`), offline `verify`, evidence-scope card |
